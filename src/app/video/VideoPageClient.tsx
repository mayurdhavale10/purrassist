...
export default function VideoPageClient() {
  ...
  // Keep latest values without re-running effects
  ...
  const fetchUserPlan = async () => {
    try {
      ...
      // Generate matching options and set ONCE (prevents extra re-renders)
  ...
  useEffect(() => {
    if (showVideo) {
      (async () => {
        ...
        // If already matched in text-only, start WebRTC when video turns on
  ...
  const initializeCamera = async () => {
    try {
      ...
      if (localVideoRef.current) {
        ...
        // Mirror fix: always show original orientation
  ...
  // On unmount, be sure to stop camera
  useEffect(() => {
    return () => { stopCamera(); };
  }, []);
  ...
  // SOCKET — stable: DO NOT depend on userPlan to avoid teardown that stops camera
  useEffect(() => {
    ...
    return () => {
      ...
      teardownPeer();
      // IMPORTANT: do NOT stopCamera() here; it caused the 2–3s shutdown when the effect re-ran.
      setIsRegistered(false);
      setHasEmittedRegister(false);
      setPendingStart(false);
    };
  }, [session?.user?.email, hasEmittedRegister]); // no userPlan here
  ...
  async function startPeer(partner: string, myRole: Role) {
    ...
    pcRef.current.ontrack = (e) => {
      ...
      lastRemoteStreamRef.current = stream; // remember across toggles
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden">
      ...
      <div className={`relative z-10 ${isMobile ? "p-4 space-y-4" : "p-6 min-h-screen flex gap-6"} max-w-7xl mx-auto`}>
        {/* Video Section (kept mounted; hidden when off) */}
      ...
      <style jsx>{`
        ...
        /* Contain scroll inside messages (prevents page jump while typing) */
