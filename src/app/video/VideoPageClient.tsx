...
export default function VideoPageClient() {
  ...
  // --- Camera lifecycle & toggle handling ---
  useEffect(() => {
    (async () => {
      if (showVideo) {
  ...
  const initializeCamera = async () => {
    try {
      ...
      if (localVideoRef.current) {
        ...
        // mirror FIX: show true orientation (not mirrored)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 relative overflow-hidden">
          ...
          {userPlan && (
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-4 border border-white/20 shadow-2xl">
              <h4 className="text-white font-bold text-base mb-3">
                <span className="text-lg">🎯</span> Matching Preferences
              </h4>
      ...
      <style jsx>{`
        ...
        /* Local preview: force non-mirrored (true camera orientation) */
